...
if not os.path.exists('Privates.py'):
    CLIENT_ID = input("Enter your Vito API client ID: ")
    CLIENT_SECRET = input("Enter your Vito API client secret: ")
...
# Please input the names to be used for calling this private assistant. Multiple names are possible.
systemName = ["헤이", "어이"]

# Please input your own GPT prompts, as this will affect all of the responses generated by the GPT.
systemRole = "간결한 대답."

# Please input your own words that you do not want GPT to respond to and execute custom commands
myCommands = [
        "창모드",
        "음소거",
        "계속 말할게",
...
class main:
    ...
    @staticmethod
    def bCheck_substrings(string, substrings):
    ...
    def executeCommand(request):
        ...
        if not main.nameCalled:
            main.nameCalled = main.bCheck_substrings(request, systemName)
            if main.nameCalled:
                TTS.tts("무엇을 도와드릴까요?")
            main.isProcessing = False
            return
        
        ownRequest = main.check_substrings(request, myCommands)
        if ownRequest is not None:
            # Make your own processes
            print(ownRequest)
            CustomCommand(ownRequest)
        else: # Run ChatGPT
            ...
            if main.windowMode:
                window = TextWindow(text = assistant_content)
                window.start()

        main.isProcessing = False
        main.nameCalled = False
...
class TTS:
    ...
    def tts(response):
        if TTS.engine is None:
            ...
            TTS.engine.setProperty('rate', 200)
...
class CustomCommand:
    def __init__(self, command):
        if command == "창모드":
            if main.windowMode:
                TTS.tts("창모드 비활성화")
                main.windowMode = False
            else:
                main.windowMode = True
                TTS.tts("창모드 활성화")
        elif command == "음소거":
            if not main.windowMode:
                TTS.tts("음소거 모드는 창모드일때만 가능합니다.")
                return
            if main.mute:
                TTS.tts("음소거 비활성화")
                main.mute = False
...
if __name__ == "__main__":
    ...
    print("Private Assistant Actived.")
    print("You can call me as : " + ' or '.join(systemName))
    print("Custom commands : " + ", ".join(myCommands))
